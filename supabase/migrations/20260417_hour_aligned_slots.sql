-- Add hour-aligned slots toggle: when ON, only offer appointments starting
-- at :00 every hour (9:00, 10:00, 11:00...). When OFF (default), keep the
-- existing 30-minute cadence.

ALTER TABLE booking_settings
  ADD COLUMN IF NOT EXISTS hour_aligned_slots BOOLEAN DEFAULT false;

-- Update get_available_slots RPC (used by the public patient flow) to respect
-- the new flag. The skipNotice API branch does its own filtering in TypeScript.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(bs.hour_aligned_slots, false)
  INTO v_buffer_before, v_buffer_after, v_min_notice_hours, v_hour_aligned
  ...
  -- Hour-aligned filter: only keep slots that start at :00 in the practitioner's
  -- local timezone. Slots at :30, :15, :45 etc. are dropped.
  AND (
    NOT v_hour_aligned
    OR EXTRACT(MINUTE FROM (ts.slot_time AT TIME ZONE v_timezone)) = 0
  )
