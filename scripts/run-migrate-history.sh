#!/usr/bin/env bash
# Migrate legacy History data → new "About patient" sections.
# Reads Supabase URL + service-role key from .env.local.
#
#   bash scripts/run-migrate-history.sh           # DRY RUN
#   bash scripts/run-migrate-history.sh --apply   # write
set -euo pipefail
cd "$(dirname "$0")/.."
...
exec npx tsx scripts/migrate-history-to-sections.ts "$@"
