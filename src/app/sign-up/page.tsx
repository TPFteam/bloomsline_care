...
function SignUpContent() {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Check eligibility on server
      const res = await fetch('/api/auth/magic-link', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: magicEmail.trim(), flow: 'signup' }),
      })
      const data = await res.json()

      if (!res.ok) {
        if (data.error === 'account_exists') {
          router.push(`/sign-in?info=${encodeURIComponent(data.message)}`)
        } else if (data.error === 'not_eligible') {
          router.push(`/early-access?info=${encodeURIComponent(data.message)}`)
        } else {
            ...
            {magicSent ? (
              <div className="text-center py-3">
                ...
                <p className="text-xs text-neutral-500">
                  {locale === 'fr'
                    ? `Lien d'inscription envoyé à ${magicEmail}`
                    : locale === 'es'
                      ? `Enlace enviado a ${magicEmail}`
                      : `Sign-up link sent to ${magicEmail}`}
                </p>
                <button
