...
export function RichTextEditor({ value, onChange, placeholder, memberId, locale, autoFocus, milestones, noteTypes, memberName, onAutoSave, autoSaveDelay = 2000, toolbarActions, compact, onSubmit, lockedTypes, onAddType, onRenameType, onDeleteType, getTagNoteCount, maxTypes = 10, onTagInsert, activeTagType }: RichTextEditorProps) {
          ...
          if (editor) {
            ...
            if (sel && sel.rangeCount) {
              if (inlineHighlight === 0 && inlineTrigger.source !== '/') {
                ...
                const saidHTML =
                  `<mark data-verbatim="${memberName}" data-verbatim-type="said" style="display:block;background-color:#eff6ff;border-left:4px solid #3b82f6;padding:12px 16px;margin:12px 0;border-radius:6px;font-style:italic;color:#1e3a5f;--tag-color:#3b82f6;--tag-bg:#eff6ff;">\u200B</mark>`
  ...
  const wrapSelectionWithVerbatim = useCallback(() => {
    ...
    mark.setAttribute('style',
      'display:block;background-color:#eff6ff;border-left:4px solid #3b82f6;padding:12px 16px;margin:12px 0;border-radius:6px;font-style:italic;color:#1e3a5f;--tag-color:#3b82f6;--tag-bg:#eff6ff;'
    )
    wrapWithMark(mark)
    editor.focus()
